...
from friday.core.types import Message, Role, TelemetryRecord
from friday.telemetry.aggregator import AggregatedStats, TelemetryAggregator
...
_bench_mod = importlib.import_module("friday.cli.bench_cmd")
...
def _energy_config(tmp_path, gpu_metrics=True):
    """Build a FridayConfig with energy monitoring enabled."""
    cfg = FridayConfig()
...
class TestCliAskWiring:
    ...
    def test_energy_monitor_created_when_gpu_metrics_on(
        ...
        with patch(
            "friday.telemetry.energy_monitor.create_energy_monitor",
...
class TestSdkWiring:
    ...
    def test_energy_monitor_stored(self, tmp_path):
        """Energy monitor is created and stored on Friday instance."""
        from friday.sdk import Friday
...
class TestBenchWiring:
    ...
    def test_energy_monitor_passed_to_benchmarks(self):
        ...
        with (
            patch(
                "friday.cli.bench_cmd.get_engine",
                return_value=("mock", engine),
            ),
            patch(
                "friday.cli.bench_cmd.load_config",
                return_value=cfg,
            ),
            patch(
                "friday.telemetry.energy_monitor.create_energy_monitor",
...
def _patch_telemetry_config(tmp_path: Path):
    ...
    return mock.patch(
        "friday.cli.telemetry_cmd.load_config",
        return_value=cfg,
    ), db_path
