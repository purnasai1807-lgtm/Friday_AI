...
def test_returns_tools_from_mcp_server(mock_load_config: MagicMock):
    ...
    with (
        patch("friday.mcp.transport.StreamableHTTPTransport"),
        patch("friday.mcp.client.MCPClient"),
        patch("friday.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
