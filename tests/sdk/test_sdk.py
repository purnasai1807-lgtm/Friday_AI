"""Tests for the Python SDK — Friday class and MemoryHandle."""
...
from friday.sdk import Friday, MemoryHandle
...
class TestFridayInit:
    def test_default_config(self):
        j = Friday(config=FridayConfig())
    ...
    def test_version_property(self):
        j = Friday(config=FridayConfig())
        assert j.version == friday.__version__
        j.close()

    def test_engine_key_override(self):
        j = Friday(config=FridayConfig(), engine_key="custom")
        assert j._engine_key == "custom"
        j.close()

    def test_model_override(self):
        j = Friday(config=FridayConfig(), model="my-model")
...
class TestFridayAsk:
    ...
    def test_ask_no_engine_raises(self):
        with patch("friday.sdk.get_engine", return_value=None):
            j = Friday(config=FridayConfig())
