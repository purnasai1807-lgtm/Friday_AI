"""Tests for ``friday chat`` interactive REPL command."""
...
from friday.cli.chat_cmd import _read_input, chat
...
class TestChatAgents:
    def test_simple_agent_does_not_receive_tool_only_kwargs(self) -> None:
        ...
        with (
            patch("friday.cli.chat_cmd.load_config", return_value=config),
            patch("friday.engine.get_engine", return_value=("mock", engine)),
            patch("friday.intelligence.register_builtin_models"),
        ):
