"""Tests for FridaySystem.wire_channel() — channel → agent routing.

These tests exercise wire_channel() on FridaySystem directly. The serve.py
...
def _make_system(engine=None, agent_name="", tmp_path=None) -> FridaySystem:
    """Build a minimal FridaySystem with mock engine for testing wire_channel."""
    config = FridayConfig()
...
# ---------------------------------------------------------------------------
# Tests via FridaySystem.wire_channel()
# ---------------------------------------------------------------------------
...
class TestChannelToolLoading:
    """FridaySystem receives tools when agent accepts them."""

    def test_tool_using_agent_receives_tools(self, tmp_path):
        """FridaySystem built with a tool list passes tools to the agent via ask()."""
    ...
    def test_non_tool_agent_receives_empty_tools(self, tmp_path):
        """FridaySystem with no tools list results in empty tools — simple agent
        unaffected."""
        config = FridayConfig()
        config.sessions.db_path = str(tmp_path / "sessions.db")
