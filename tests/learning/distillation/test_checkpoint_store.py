"""Tests for friday.learning.distillation.checkpoint.store module."""
...
def _setup_isolated_repo_root(tmp_path: Path) -> Path:
    """Create a fake friday-home directory tree with config files for the
    CheckpointStore to track. Returns the root."""
    root = tmp_path / "friday_home"
...
class TestCheckpointStoreInit:
    ...
    def test_creates_repo_with_baseline_commit(self, tmp_path: Path) -> None:
        from friday.learning.distillation.checkpoint.store import (
            CheckpointStore,
        )
    ...
    def test_init_refuses_inside_source_tree(
        ...
        from friday.learning.distillation.storage import paths

        source_root = paths._find_source_root()
        assert source_root is not None
        bad_root = source_root / "fake_friday_home"
