...
Index a directory of documents into Friday memory and answer questions
with context-augmented retrieval and citations.

## Requirements

- Friday installed (`git clone https://github.com/purnasai1807-lgtm/Friday_AI.git && cd Friday && uv sync` or `uv sync --extra dev`)
...
1. **Index** -- Uses `Friday.memory.index()` to chunk the documents at
...
Friday SDK. Adjust `--chunk-size` and `--top-k` to tune the
