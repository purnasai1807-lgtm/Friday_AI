...
Friday orchestrator agent.

## What This Demonstrates

- **Channel integration** — connecting Friday to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- Friday installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the Friday channel docs).
...
Friday supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/friday/config.toml` for the full list of channel and agent
options.
...
this via the Friday scheduler:

```bash
friday scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/friday/recipes/data/operators/` for examples.
