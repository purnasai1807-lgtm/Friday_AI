...
A set of developer-focused scripts that use Friday tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Friday` instance with the `native_react` (ReAct) agent
...
1. **Install Friday** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/friday/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/friday/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run friday eval list` or
inspect `src/friday/tools/` for the full registry.
