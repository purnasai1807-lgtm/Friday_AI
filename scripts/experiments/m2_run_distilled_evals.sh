...
M2_CONFIGS="src/friday/evals/configs/distillation/m2"
BASELINE_CONFIGS="src/friday/evals/configs"
M2_HOME="/scratch/user/jonsaadfalcon/friday-m2"
MODEL_FILTER=${1:-all}
BENCH_FILTER=${2:-all}
...
run_eval() {
    ...
    env OPENJARVIS_CONFIG="${oj_config}" ${VENV} -m friday.evals run -c "${config_path}" 2>&1
    local rc=$?
