...
#   - friday learning init already run
#
# Usage:
...
CONFIGS_DIR="src/friday/evals/configs/distillation"
...
check_prereqs() {
    ...
    if [ ! -d "$HOME/.friday/learning" ]; then
        log "Running friday learning init..."
        uv run friday learning init
    fi
...
run_session() {
    ...
    while [ "$session_num" -le "$n_sessions" ]; do
        ...
        # (friday learning run doesn't support all config params yet,
...
from friday.learning.distillation.checkpoint.store import CheckpointStore
from friday.learning.distillation.models import AutonomyMode
from friday.learning.distillation.orchestrator import DistillationOrchestrator
from friday.learning.distillation.storage.session_store import SessionStore
from friday.learning.distillation.student_runner import (
    VLLMStudentRunner,
    build_benchmark_samples_from_traces,
)
from friday.learning.distillation.triggers import OnDemandTrigger
from friday.learning.optimize.feedback.judge import TraceJudge

home = Path(os.environ.get("OPENJARVIS_HOME", str(Path.home() / ".friday")))
...
judge_backend = FridayDirectBackend(engine_key="cloud")
