/**
 * deep-link.ts — Handler for friday:// deep links in the Tauri desktop app.
 *
 ...
 *   friday://research/{session_id}   → open a research session
 *   friday://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `friday://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `friday://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `friday:` scheme.
 *
 * @example
 * parseDeepLink("friday://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("friday://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "friday:") return null;

    // The URL constructor treats "friday://research/abc123" such that
