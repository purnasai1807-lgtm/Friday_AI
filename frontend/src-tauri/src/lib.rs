...
/// Find the Friday project root (contains pyproject.toml).
...
fn find_project_root() -> Option<std::path::PathBuf> {
    ...
    let direct = [
        format!("{home}/Friday"),
        format!("{home}/projects/hazy/Friday"),
        format!("{home}/projects/Friday"),
        format!("{home}/src/Friday"),
        format!("{home}/Documents/Friday"),
        format!("{home}/Desktop/Friday"),
        format!("{home}/Developer/Friday"),
        format!("{home}/dev/Friday"),
        format!("{home}/Code/Friday"),
        format!("{home}/code/Friday"),
        format!("{home}/repos/Friday"),
        format!("{home}/github/Friday"),
    ];
    for p in &direct {
    ...
    // 4. Shallow scan: look for Friday one level inside common parent dirs.
    //    This catches clones like ~/Documents/my-stuff/Friday without
    ...
    for parent in &scan_parents {
        ...
        if let Ok(entries) = std::fs::read_dir(&parent_path) {
            for entry in entries.flatten() {
                let candidate = entry.path().join("Friday");
                if candidate.join("pyproject.toml").exists() {
                    return Some(candidate);
                }
                // Also check if the entry itself is Friday (case-insensitive match)
                if let Some(name) = entry.file_name().to_str() {
                    if name.eq_ignore_ascii_case("friday")
...
// ---------------------------------------------------------------------------
// BackendManager — owns the Ollama + Friday server child processes
// ---------------------------------------------------------------------------
...
struct BackendManager {
    ...
    friday: Option<ChildHandle>,
}

impl BackendManager {
    async fn stop_all(&mut self) {
        if let Some(ref mut h) = self.friday {
            h.kill().await;
        }
        self.friday = None;
...
async fn boot_backend(backend: SharedBackend, status: SharedStatus) {
    ...
    if project_root.is_none() {
        ...
        let target_path = std::path::PathBuf::from(home_dir()).join("Friday");
        ...
        if target_path.exists() && !target_path.join("pyproject.toml").exists() {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "{} exists but is not a valid Friday project. \
        ...
        {
            let mut s = status.lock().await;
            s.detail = "Downloading Friday (first launch)...".into();
        }
        ...
        let clone_result = tokio::process::Command::new(&git_bin)
            .args([
                ...
                "--depth",
                "1",
                "https://github.com/purnasai1807-lgtm/Friday_AI.git",
                &clone_target,
            ])
            .stdout(std::process::Stdio::null())
        ...
        match clone_result {
            Ok(child) => match child.wait_with_output().await {
                ...
                Ok(output) => {
                    ...
                    let mut s = status.lock().await;
                    s.error = Some(format!(
                        "Failed to download Friday: {}. \
                         Clone manually: git clone https://github.com/purnasai1807-lgtm/Friday_AI.git {}",
                        stderr.trim(),
                        clone_target,
                    ));
                    return;
    ...
    // Inject cloud API keys from ~/.friday/cloud-keys.env
    for (key, value) in read_cloud_keys() {
        cmd.env(&key, &value);
    }
    let friday_child = cmd.spawn();

    match friday_child {
        Ok(child) => {
            backend.lock().await.friday = Some(ChildHandle { child });
        }
        Err(e) => {
            let mut s = status.lock().await;
            s.error = Some(format!(
                "Could not start friday server: {}. \
                 Make sure uv is installed (https://astral.sh/uv) and the Friday repo is cloned at {}",
                e,
                root.display(),
            ));
            return;
    ...
    if !server_ok {
        ...
        let mut stderr_msg = String::new();
        {
            ...
            if let Some(ref mut h) = mgr.friday {
        ...
        let detail = if stderr_msg.is_empty() {
            format!(
                "Friday server did not start. Check that:\n\
                 1. uv is installed ({})\n\
                 2. The Friday repo is at {}\n\
...
#[tauri::command]
async fn run_friday_command(args: Vec<String>) -> Result<String, String> {
    let mut cmd_args = vec!["run".to_string(), "friday".to_string()];
    ...
    let output = tokio::process::Command::new(&uv_bin)
        ...
        .output()
        .await
        .map_err(|e| format!("Failed to launch friday: {}", e))?;
...
/// Path to the cloud keys file (~/.friday/cloud-keys.env).
...
mod native_overlay {
    ...
    pub unsafe fn create(html: &str, api_port: u16) {
        ...
        if Class::get("FridayOverlayPanel").is_none() {
            let sup = Class::get("NSPanel").unwrap();
            let mut decl = ClassDecl::new("FridayOverlayPanel", sup).unwrap();
        ...
        if Class::get("FridayOverlayNavDelegate").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("FridayOverlayNavDelegate", sup).unwrap();
        ...
        if Class::get("FridayOverlayMsgHandler").is_none() {
            let sup = Class::get("NSObject").unwrap();
            let mut decl = ClassDecl::new("FridayOverlayMsgHandler", sup).unwrap();
        ...
        let cls = Class::get("FridayOverlayPanel").unwrap();
        ...
        let hcls = Class::get("FridayOverlayMsgHandler").unwrap();
        ...
        let nav_cls = Class::get("FridayOverlayNavDelegate").unwrap();
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let health = MenuItemBuilder::with_id("health", "Health: starting...")
                .enabled(false)
                .build(app)?;
            let quit = MenuItemBuilder::with_id("quit", "Quit Friday").build(app)?;
        ...
        .expect("error while building Friday Desktop")
        .run(move |_app, event| {
