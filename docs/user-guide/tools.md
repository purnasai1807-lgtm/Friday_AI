...
# Execute a tool call
from friday.core.types import ToolCall
...
```python
from friday.tools._stubs import build_tool_descriptions
...
from friday.memory.sqlite import SQLiteMemory
...
```python
from friday.scheduler.tools import ScheduleTaskTool
from friday.scheduler.scheduler import TaskScheduler
from friday.scheduler.store import SchedulerStore

store = SchedulerStore(db_path="~/.friday/scheduler.db")
scheduler = TaskScheduler(store=store, system=friday_system)
scheduler.start()
...
```bash
friday ask --agent orchestrator --tools my_tool "Process this data"
```
...
# Multiple tools
friday ask --agent orchestrator --tools calculator,think "Solve: 2x + 5 = 13"

# All available tools (list them)
friday ask --agent orchestrator --tools calculator,think,retrieval,file_read "..."
```
