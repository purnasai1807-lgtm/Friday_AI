...
This creates the distillation directory layout under `~/.friday/learning/` and initializes a git checkpoint repo at `~/.friday/.git` for tracking config changes.
...
| **review** | Queued for user approval in `friday learning review` | System prompt edits, agent class changes, few-shot exemplars |
...
friday learning reject <edit-id> --reason "prompt change too aggressive"
```
...
Every edit creates a git commit in the checkpoint repo at `~/.friday/.git`. This is separate from your Friday source repo.

```bash
# List past sessions
friday learning history --limit 20
...
Cost is tracked on every `LearningSession` as `teacher_cost_usd` and surfaced in `friday learning show`.
...
```bash
# Manual refresh
friday learning benchmark refresh

# Show stats
friday learning benchmark show
```
...
- **< 20 traces:** `friday learning run` returns "Not enough traces yet." Triggers are no-ops.
...
**Getting there faster:** Use Friday normally and provide feedback on results (thumbs up/down in the UI, or `friday feedback` in the CLI).

## Troubleshooting

| Error | Cause | Fix |
|-------|-------|-----|
| "Not enough traces yet" | Fewer than 20 traces in the store | Use Friday more, provide feedback |
| "Working tree dirty, cannot stage" | Manual edits to `~/.friday/config.toml` during a session | Commit or revert manual changes first |
...
| "ConfigurationError: distillation root inside source tree" | `OPENJARVIS_HOME` points inside the repo | Set `OPENJARVIS_HOME` to `~/.friday` (default) or another external dir |
| "Personal benchmark is empty" | Not enough high-feedback traces yet | Provide feedback on 10+ traces with score >= 0.7 |

## Where Artifacts Live

All distillation artifacts live under `~/.friday/` (never inside the source repo):
...
```bash
friday learning daemon start    # Start background watcher
friday learning daemon status   # Check if running
friday learning daemon stop     # Stop the daemon
```
...
- [User Guide: CLI](cli.md#friday-learning) — full CLI reference
