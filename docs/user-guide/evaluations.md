...
The Friday evaluation framework (`friday-evals`) measures model **correctness and accuracy** on academic datasets. It is a separate package from the main Friday library and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    Friday has two distinct measurement systems that complement each other:

    | System | Package | Measures | Entry Point |
    |--------|---------|----------|-------------|
    | **Evaluations** | `friday-evals` | Correctness on academic datasets (accuracy, pass rate) | `friday-eval` |
    | **Benchmarks** | `friday` | Engine performance (latency, throughput) | `friday bench` |
...
The evaluation framework is a standalone package in the `evals/` directory. Install it alongside Friday:

```bash
uv sync --extra eval
```

This installs the `friday-eval` CLI entry point and all required dependencies (`datasets`, `huggingface-hub`, `tqdm`, `rich`).
...
These benchmarks evaluate models on practical tasks that mirror real Friday use cases.
...
```bash
uv run python -m friday.evals --config src/friday/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run python -m friday.evals --config src/friday/evals/configs/use_case_v2_local.toml
```
...
|---------|-----|-------------|
| **friday-direct** | `friday-direct` | Engine-level inference via `SystemBuilder`. Works for local (Ollama, vLLM, llama.cpp) and cloud models. |
| **friday-agent** | `friday-agent` | Agent-level inference with tool calling. Uses `FridaySystem.ask()` with the specified agent and tools. |

Use `friday-direct` for most evaluations. Use `friday-agent` when the benchmark requires tool use — for example, GAIA tasks that reference files that must be read with `file_read`, or arithmetic tasks that benefit from `calculator`.
...
| `--backend` | | choice | `friday-direct` | `friday-direct` or `friday-agent` |
| `--model` | `-m` | str | required* | Model identifier (e.g., `qwen3:8b`, `gpt-4o`) |
| `--engine` | `-e` | str | auto | Engine key (`ollama`, `vllm`, `cloud`, ...) |
| `--agent` | | str | `orchestrator` | Agent name for `friday-agent` backend |
...
```bash
friday-eval run-all -m qwen3:8b

# With options
friday-eval run-all -m gpt-4o -n 100 --output-dir results/gpt4o/
```
...
```bash
friday-eval summarize results/supergpqa_qwen3-8b.jsonl
```
...
```bash
friday-eval run --config src/friday/evals/configs/full-suite.toml
```
...
| `backend` | str | `"friday-direct"` | Inference backend: `friday-direct` or `friday-agent` |
...
| `agent` | str | `None` | Agent name for `friday-agent` backend (e.g., `"orchestrator"`) |
| `tools` | list[str] | `[]` | Tool names for `friday-agent` backend |
...
friday-eval run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Agents](agents.md) — Configure the `OrchestratorAgent` used by `friday-agent` backend
- [Tools](tools.md) — Available tools for agent-backed evaluations
- [Python SDK](python-sdk.md) — Programmatic access to Friday inference and agents
