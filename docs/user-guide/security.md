...
Friday includes a security layer that scans prompts and model outputs for secrets, personally identifiable information (PII), and sensitive file paths. The system is designed to be composable: scanners run as a pipeline, and the `GuardrailsEngine` wrapper drops in front of any inference backend without changing how the rest of your code works.
...
=== "Warn mode (default)"
    ...
    from friday.engine.ollama import OllamaEngine
    from friday.security.guardrails import GuardrailsEngine
    from friday.security.types import RedactionMode
...
=== "Block mode"
    ...
    from friday.engine.ollama import OllamaEngine
    from friday.security.guardrails import GuardrailsEngine, SecurityBlockError
    from friday.security.types import RedactionMode
    from friday.core.types import Message, Role
...
from friday.security.guardrails import GuardrailsEngine
from friday.security.scanner import SecretScanner
from friday.security.types import RedactionMode
...
from friday.security.scanner import PIIScanner
...
from friday.security.file_policy import is_sensitive_file, filter_sensitive_paths
...
audit = AuditLogger(db_path="~/.friday/audit.db", bus=bus)
...
import time
from friday.security.audit import AuditLogger
from friday.security.types import SecurityEvent, SecurityEventType
...
audit = AuditLogger(db_path="~/.friday/audit.db")
...
Security settings live in the `[security]` section of `~/.friday/config.toml`.
...
secret_scanner = true
pii_scanner = true
audit_log_path = "~/.friday/audit.db"
...
| `audit_log_path` | `str` | `~/.friday/audit.db` | Path to the SQLite audit log |
...
import re
from friday.security._stubs import BaseScanner
from friday.security.types import ScanFinding, ScanResult, ThreatLevel
...
- [API Reference: Security](../api-reference/friday/security/index.md) — full class and function signatures
