...
# Spotify
friday connect spotify

# Strava
friday connect strava
```
...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.friday/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.friday/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.friday/connectors/news_rss.json << 'EOF'
{"feeds": [
...
```bash
CARTESIA_API_KEY="sk_car_..." friday digest --fresh
```
...
```bash
friday digest --fresh          # Generate a new digest now
friday digest                  # Show today's cached digest
friday digest --text-only      # Print text without audio
friday digest --history        # Show past digests
friday digest --schedule "0 6 * * *"   # Set daily schedule
friday digest --schedule off   # Disable schedule
friday digest --schedule       # Show current schedule
```

## Saying "Good morning"

When chatting with Friday (via CLI, desktop, or browser), saying "Good morning" or "morning digest" automatically triggers the digest — no need to use the `digest` command explicitly.
...
The `persona` field loads a prompt file from `configs/friday/prompts/personas/{name}.md`. The default `friday` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.friday/connectors/news_rss.json`:
...
**"No digest for today"** — Run `friday digest --fresh` to generate one.

**Empty sections** — Check connector status with `friday connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
