...
- **Persistence:** SQLite database file (default: `~/.friday/memory.db`)
...
```python
from friday.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from friday.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `friday.tools.storage.*` imports.
...
[Source: docs/intro.md] Friday is a modular AI framework...
...
friday memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
friday memory search "machine learning"
