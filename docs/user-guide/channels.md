...
The channels module lets Friday send and receive messages through external messaging platforms. Each platform has a dedicated channel implementation that connects directly to the platform's API -- there is no intermediate gateway.
...
from friday.channels.telegram import TelegramChannel
...
from friday.channels._stubs import ChannelMessage
from friday.channels.discord_channel import DiscordChannel
...
from friday.channels.slack import SlackChannel
...
from friday.core.events import EventBus, EventType
from friday.channels.telegram import TelegramChannel
...
The `friday channel` subcommand group provides quick access to channel operations.
...
friday channel send telegram "Build completed successfully"
```
...
When `friday serve` is running, three channel endpoints are available. Channels must be configured and enabled in `[channel]` for these endpoints to return data.
...
Channel settings live in the `[channel]` section of `~/.friday/config.toml`. Each platform has its own nested sub-section.

```toml title="~/.friday/config.toml"
[channel]
enabled = true
default_channel = ""
...
# Send a message
if channels:
    channel.send(channels[0], "Hello from Friday!")
...
```
Your phone  ──text──▶  SendBlue  ──webhook──▶  ngrok tunnel  ──▶  Friday
                                                                       │
Your phone  ◀──iMessage──  SendBlue  ◀──API call──  DeepResearch agent ◀┘
```

When someone texts the SendBlue number, SendBlue POSTs the message to your webhook. Friday sends an immediate "Message received!" acknowledgment, runs the DeepResearch agent, and sends the response back via iMessage.
...
Since Friday runs locally, you need a tunnel so SendBlue can reach your webhook:
...
from friday.channels.sendblue import SendBlueChannel
...
# Send a message
ok = channel.send("+15551234567", "Hello from Friday!")
```
...
friday channel status --channel-type sendblue

# Send a message
friday channel send sendblue "+15551234567" "Hello from Friday!"
```

### Server Restart Behavior

SendBlue bindings are **automatically restored on server restart**. When `friday serve` starts:
...
The Baileys bridge is a compiled Node.js application bundled inside the package. It is auto-installed to `~/.friday/whatsapp_baileys_bridge/` on first `connect()` call. If `node` is not found on `PATH`, `connect()` logs an error and sets the channel to `ChannelStatus.ERROR`.
...
from friday.channels.whatsapp_baileys import WhatsAppBaileysChannel

channel = WhatsAppBaileysChannel(
    assistant_name="Friday",           # (1)!
...
On first connection, the bridge will print a QR code to the terminal. Scan it with the WhatsApp app on your phone to authenticate. Authentication state is saved to `~/.friday/whatsapp_baileys_bridge/auth/` and reused on subsequent connections.
...
| Parameter                  | Type       | Default                                  | Description                                           |
| -------------------------- | ---------- | ---------------------------------------- | ----------------------------------------------------- |
| `auth_dir`                 | `str`      | `~/.friday/whatsapp_baileys_bridge/auth` | Baileys auth state directory                          |
| `assistant_name`           | `str`      | `"Friday"`                               | Display name for the assistant                        |
...
auth_dir = "/home/user/.friday/whatsapp_baileys_bridge/auth"
...
- [API Reference: Channels](../api-reference/friday/channels/index.md) -- full class and type signatures
