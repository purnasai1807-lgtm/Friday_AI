...
A lightweight conversational AI with no tools and no agent overhead. This is the simplest possible Friday setup: just Ollama and a local model. Ideal for general-purpose chat, Q&A, brainstorming, and getting started quickly.
...
### 2. Install and initialize Friday

```bash
git clone https://github.com/purnasai1807-lgtm/Friday_AI.git
cd Friday
uv sync
friday init --preset chat-simple
```

### 3. Ask a question

```bash
friday ask "What is quantum computing?"
```
...
```bash
# Single question
friday ask "Explain the difference between TCP and UDP"
...
friday ask -m qwen3.5:9b "Explain general relativity"

# Adjust temperature (0.0 = deterministic, 1.0 = creative)
friday ask -t 0.2 "List the planets in our solar system"

# Output raw JSON
friday ask --json "What is 2+2?"
```
...
To switch models, either edit `~/.friday/config.toml` or override per-query:

```bash
friday ask -m qwen3.5:35b "Write a detailed comparison of REST and GraphQL"
```
...
1. Start the backend: `friday serve` (or `./scripts/quickstart.sh`)
2. Download and open the desktop app from the [releases page](https://github.com/purnasai1807-lgtm/Friday_AI/releases)
...
${EDITOR:-nano} ~/.friday/config.toml
...
friday ask -m deepseek-r1:14b "Hello"
```

**Use an environment variable:**

```bash
OPENJARVIS_MODEL=qwen3.5:9b friday ask "Hello"
```
...
**Browser app not loading** -- Make sure both the backend (`friday serve`) and frontend are running. The `./scripts/quickstart.sh` script starts both automatically.
