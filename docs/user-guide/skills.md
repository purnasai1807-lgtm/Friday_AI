...
| **Overlay** | Sidecar file at `~/.friday/learning/skills/` storing optimized descriptions and few-shot examples |
...
friday skill install hermes:apple-notes

# Bulk install a category
friday skill sync hermes --category research

# Run a skill directly
friday skill run math-solver -a expression="41 + 82"

# See skill details
friday skill info research-and-summarize
```
...
friday skill sync hermes --category research
friday skill sync hermes --category coding
friday skill sync hermes  # everything (~150 skills)
```
...
friday skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
friday skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
friday skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```

### Config-Driven Auto Import

Add sources to `~/.friday/config.toml` for automatic syncing:
...
Friday can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
friday skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.friday/skills/discovered/
friday skill discover
```

Discovered skills land in `~/.friday/skills/discovered/` and automatically appear in `friday skill list` on the next session.
...
friday optimize skills --dry-run

# Run DSPy optimization
friday optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
friday optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
friday skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.friday/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
# Single condition
friday bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with Friday | Implicit trust | Full access within declared capabilities |
...
```bash
friday skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.friday/skills/"  # where skills are installed
...
overlay_dir = "~/.friday/learning/skills/"
```
...
2. **User** `~/.friday/skills/`
3. **Bundled** (shipped with Friday)

## CLI Reference

| Command | Description |
|---------|-------------|
| `friday skill list` | List installed skills |
| `friday skill info <name>` | Show detailed skill information |
| `friday skill run <name> [-a key=value]` | Execute a skill directly |
| `friday skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `friday skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `friday skill sources` | List configured skill sources |
| `friday skill update` | Pull latest from configured sources |
| `friday skill remove <name>` | Remove an installed skill |
| `friday skill search <query>` | Search the skill index |
| `friday skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `friday skill show-overlay <name>` | Inspect optimization output for a skill |
| `friday optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `friday bench skills [--condition C]` | Run the PinchBench skills benchmark |
