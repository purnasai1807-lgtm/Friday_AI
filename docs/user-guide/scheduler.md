...
!!! note "Optional component"
    The scheduler is a standalone module (`friday.scheduler`). It is not wired into the default `Friday` / `FridaySystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `friday scheduler start`.
...
The `friday scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
friday scheduler list --status active

# Paused tasks
friday scheduler list --status paused
```
...
friday scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
friday scheduler resume a3f9b12c4d8e
```
...
friday scheduler cancel a3f9b12c4d8e
```
...
friday scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.friday/scheduler.db")  # (1)!

# Wire in a FridaySystem for task execution
...
4. Starts a daemon thread named `"friday-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.friday/config.toml`.

```toml title="~/.friday/config.toml"
[scheduler]
enabled = false
db_path = "~/.friday/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.friday/scheduler.db`     | SQLite database path                       |
