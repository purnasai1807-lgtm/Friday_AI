...
Friday has two types of integrations:
...
```json
{
    "display_information": { "name": "Friday" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "Friday", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
- You DM @Friday in Slack → Socket Mode receives the event in real-time
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "Friday" |
...
3. **Connect in Friday:**
   ...
   - CLI: `uv run friday connect gmail_imap`
...
Slack serves two purposes in Friday:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [Friday icon](https://github.com/purnasai1807-lgtm/Friday_AI/blob/main/assets/friday-slack-icon.jpg)
...
**DM your agent:**
- In Slack, find **Friday** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "Friday"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @Friday` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @Friday` in the channel |
...
3. **Connect in Friday:**
   ...
   - CLI: `uv run friday connect notion`
...
3. **Connect in Friday:**
   ...
   - CLI: `uv run friday connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the Friday desktop app
...
3. **Connect in Friday:**
   ...
   - CLI: `uv run friday connect outlook`
...
2. **Connect in Friday:**
   ...
   - CLI: `uv run friday connect obsidian --path /path/to/vault`
...
4. **Connect in Friday:**
   ...
   - CLI: `uv run friday connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run friday deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.friday/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.friday/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
