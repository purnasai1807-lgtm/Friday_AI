...
!!! note "Before you begin"
    This tutorial assumes Friday is installed with Ollama running and a model available (e.g., `qwen3.5:9b`). If you have not completed setup yet, start with the [Quick Start guide](../getting-started/quickstart.md).

## Step 1: Install Skills from Hermes Agent

Friday can import skills from the [Hermes Agent](https://github.com/NousResearch/hermes-agent) skill library maintained by NousResearch. Let's install a few useful ones.

```bash
# Install individual skills
friday skill install hermes:arxiv
friday skill install hermes:github-pr-workflow

# Or bulk install an entire category
friday skill sync hermes --category research
```

The first install clones the Hermes repo to `~/.friday/skill-cache/hermes/` (one-time, ~5s). Subsequent installs reuse the cache.
...
```bash
cat ~/.friday/skills/hermes/arxiv/SKILL.md | head -40
```

The `.source` file records provenance:

```bash
cat ~/.friday/skills/hermes/arxiv/.source
```
...
```bash
friday ask "Use the math-solver skill to compute 17 * 23" \
  --engine ollama --model qwen3.5:9b
```
...
```bash
mkdir -p ~/.friday/skills/my-reviewer
```

Write a SKILL.md:

```bash
cat > ~/.friday/skills/my-reviewer/SKILL.md << 'EOF'
---
name: my-reviewer
...
```bash
friday ask "Use the my-reviewer skill to review this function: def login(user, pwd): return db.query(f'SELECT * FROM users WHERE name={user} AND pass={pwd}')" \
  --engine ollama --model qwen3.5:9b
```
...
friday ask "Use math-solver to compute 100 / 7"
friday ask "Use code-explainer to explain: lambda x: x**2"
friday ask "Use my-reviewer to review: def add(a,b): return a+b"
friday ask "Use math-solver to compute 2**10"
friday ask "Use code-explainer to explain: [x for x in range(10) if x % 2 == 0]"
```

Each query produces a trace in `~/.friday/traces.db` with skill metadata tags (`skill`, `skill_source`, `skill_kind`).
...
friday skill discover --dry-run --min-frequency 2

# Write discovered patterns as skill manifests
friday skill discover --min-frequency 2
```
...
This produces overlay files at `~/.friday/learning/skills/<skill-name>/optimized.toml` with improved descriptions and few-shot examples extracted from your best traces.

Inspect what was produced:

```bash
friday skill show-overlay math-solver
friday skill show-overlay code-explainer
```
...
For a hands-off experience, add this to `~/.friday/config.toml`:
...
|---------|-------------|
| **Installing skills** | `friday skill install hermes:arxiv` — imported from public sources |
| **Using skills** | `friday ask "Use the code-explainer skill..."` — agent invokes skills as tools |
...
| **Discovering patterns** | `friday skill discover` — mined traces for recurring tool sequences |
| **Optimizing skills** | `friday optimize skills --policy dspy` — improved descriptions + few-shot examples |
| **Benchmarking** | `friday bench skills` — measured the impact across 4 conditions |
