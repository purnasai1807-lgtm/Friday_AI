...
Friday provides Docker images for both CPU-only and GPU-accelerated deployments, along with a Docker Compose configuration that bundles the API server with an Ollama inference backend.

## Quick Start

The fastest way to get Friday running in Docker is with Docker Compose, which starts both the API server and an Ollama backend:
...
1. **Builder stage** -- installs `uv` and the `friday[server]` package (which includes FastAPI, uvicorn, and all server dependencies) from the project source.
...
```bash
docker run -d -p 8000:8000 friday:latest
```
...
```bash
docker build -f Dockerfile.gpu -t friday:gpu .
```
...
```bash
docker run -d --gpus all -p 8000:8000 friday:gpu
```
...
The `docker-compose.yml` defines a complete deployment with the Friday API server and an Ollama backend:
...
The `friday` service is configured through environment variables:
...
The `friday` service declares `depends_on: ollama`, ensuring the Ollama container starts before the API server. Both services use `restart: unless-stopped` to automatically recover from crashes.

## Custom Configuration

### Mounting a Configuration File

To use a custom `config.toml`, mount it into the container at the expected path (`~/.friday/config.toml`, which is `/root/.friday/config.toml` in the container):
...
services:
  friday:
    ...
    ports:
      - "8000:8000"
    volumes:
      - ./my-config.toml:/root/.friday/config.toml:ro
    environment:
      - OPENJARVIS_ENGINE_DEFAULT=ollama
      - OPENJARVIS_OLLAMA_HOST=http://ollama:11434
    depends_on:
...
To persist telemetry data, memory databases, and trace records across container restarts, mount the entire Friday data directory:
...
The entrypoint is `friday` and the default command is `serve --host 0.0.0.0 --port 8000`. Override the command to change server options:

```bash
docker run -d -p 9000:9000 friday:latest \
...
### Available CLI Options for `friday serve`
