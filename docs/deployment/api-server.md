...
Friday includes an OpenAI-compatible API server built on FastAPI and uvicorn. It exposes chat completion, model listing, and health check endpoints, making it a drop-in replacement for the OpenAI API when working with local models.
...
The server reads defaults from `~/.friday/config.toml` and auto-detects available engines and models. Override any option via CLI flags:
...
If the `[server]` extra is not installed, `friday serve` exits with a clear error message explaining how to install the required dependencies.
...
The `[server]` section of `~/.friday/config.toml` controls default server behavior:
...
| `agent`   | `string`  | `"autonomous"` | Default agent for non-streaming requests. Set to `""` for direct engine mode.                                             |
...
CLI flags override config file values. For example, `friday serve --port 9000` overrides the `port` setting in the config file.
...
For production deployments, run Friday behind a reverse proxy like Nginx or Caddy for TLS termination, rate limiting, and authentication.
...
server {
    ...
    ssl_certificate /etc/ssl/certs/friday.pem;
    ssl_certificate_key /etc/ssl/private/friday.key;
...
```bash
friday serve --host 127.0.0.1 --port 8000
```
