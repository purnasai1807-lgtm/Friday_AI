...
```bash
uv run friday --version   # Should print 0.1.0
uv run friday --help      # Show all subcommands
```
...
Friday uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=friday --cov-report=html
```
...
Friday uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/friday/`:
...
src/friday/
    ...
    core/                       # Shared infrastructure
        config.py               # FridayConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # friday ask
        init_cmd.py             # friday init
        model.py                # friday model list/info
        memory_cmd.py           # friday memory index/search/stats
        telemetry_cmd.py        # friday telemetry stats/export/clear
        bench_cmd.py            # friday bench run
        serve.py                # friday serve
```
...
try:
    import friday.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
...
- Absolute imports only (`from friday.core.registry import ...`)
