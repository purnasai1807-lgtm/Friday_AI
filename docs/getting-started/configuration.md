---
title: Configuration
description: Complete reference for Friday configuration
---

# Configuration

Friday uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
Friday creates the `~/.friday/` directory and populates it with a default config when you run `friday init`.
...
3. Writes `~/.friday/config.toml` with sensible defaults
...
| --------- | ------ | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `friday init` based on hardware detection. |
...
!!! tip "Engine fallback"
If the configured default engine is unreachable, Friday automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, Friday uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
| Field                 | Type   | Default        | Description                                                                                                         |
| --------------------- | ------ | -------------- | ------------------------------------------------------------------------------------------------------------------- |
| `default_agent`       | string | `"autonomous"` | Default agent to use. Available: `autonomous`, `simple`, `orchestrator`, `react`, `operative`, `monitor_operative`. |
| `max_turns`           | int    | `10`           | Maximum number of tool-calling turns for the  before it must produce a final answer.                |
...
```bash
friday ask --router heuristic "Hello"
```
...
| `db_path`            | string | `~/.friday/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend.                  |
...
Controls the OpenAI-compatible API server started by `friday serve`.
...
```bash
friday serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.friday/traces.db"
```

| Field     | Type   | Default               | Description                                          |
| --------- | ------ | --------------------- | ---------------------------------------------------- |
| `enabled` | bool   | `false`               | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.friday/traces.db` | Path to the SQLite trace database.                   |
...
| `skills_dir`        | string | `~/.friday/skills/` | Directory where skills are installed.                                                                             |
...
| `overlay_dir`                   | string | `~/.friday/learning/skills/` | Where optimized skill overlays are stored.                                     |
...
| `default_agent`   | string | `"autonomous"` | Default agent for handling channel messages. |
...
When you run `friday init`, Friday probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.friday/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure Friday entirely from Python without a TOML file:
...
```python
j = Friday(config_path="/path/to/my-config.toml")
```
...
Friday respects the following environment variables:
