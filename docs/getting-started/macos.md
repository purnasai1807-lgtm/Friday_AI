...
description: Complete step-by-step guide to installing Friday on macOS with llama.cpp, including common pitfalls and fixes
search:
  boost: 2
---
...
This guide walks through a complete Friday installation on macOS using **llama.cpp** as
...
| Git | Clones the Friday repo |
| Node.js | Required for the browser UI |
| Rust | Compiles the Friday security and memory extension |
| llama.cpp | Local inference engine that runs GGUF model files |
| Friday | The framework itself |
...
`uv` replaces pip, virtualenv, and pyenv in one tool. Friday uses it to manage Python
...
Git is used to clone the Friday source code. It may already be present if you have
...
Friday includes a Rust extension that provides security scanning, memory indexing,
...
!!! warning "Do not clone from inside an existing Friday folder"
    A common mistake is running `git clone` while already inside the repo, creating deeply
    nested duplicates (`Friday/Friday/Friday`). Always clone from `~` or a
...
!!! warning "Critical step — do not skip"
    Friday requires Python 3.10–3.13. Its Rust extension uses PyO3, which does not yet
    ...
    with `ModuleNotFoundError: No module named 'friday_rust'`.
...
```bash
uv run python -c "import friday_rust; print('Rust extension OK')"
```
...
Friday needs a GGUF model file to run inference. First install the Hugging Face CLI,
...
✓  Config file            ~/.friday/config.toml
...
# Terminal 2 — backend
cd ~/Friday && uv run friday serve --port 8000

# Terminal 3 — frontend
cd ~/Friday/frontend && npm run dev
```
...
Your prompt will show `(friday)` when active, and you can type `friday ask "..."` directly.
...
2. Rebuild: `uv run maturin develop -m rust/crates/friday-python/Cargo.toml`
...
llama-server is not running. Start it in a separate terminal before running any `friday`
...
- [Architecture](../architecture/overview.md) — Understand how Friday is structured
