---
title: Installation
description: Get Friday running — browser app, desktop app, CLI, or Python SDK
search:
  boost: 3
---
...
Friday runs entirely on your hardware. Choose the interface that fits your workflow.
...
The desktop app is a native window for the Friday chat UI. All inference and backend
...
| Platform | Download |
|----------|----------|
| macOS (Apple Silicon) | [:material-download: **Friday.dmg**](https://github.com/purnasai1807-lgtm/Friday_AI/releases/download/desktop-latest/Friday_0.1.0_aarch64.dmg) |
| Windows (64-bit) | [:material-download: **Friday-setup.exe**](https://github.com/purnasai1807-lgtm/Friday_AI/releases/download/desktop-latest/Friday_0.1.0_x64-setup.exe) |
| Linux (DEB) | [:material-download: **Friday.deb**](https://github.com/purnasai1807-lgtm/Friday_AI/releases/download/desktop-latest/Friday_0.1.0_amd64.deb) |
| Linux (RPM) | [:material-download: **Friday.rpm**](https://github.com/purnasai1807-lgtm/Friday_AI/releases/download/desktop-latest/Friday-0.1.0-1.x86_64.rpm) |
| Linux (AppImage) | [:material-download: **Friday.AppImage**](https://github.com/purnasai1807-lgtm/Friday_AI/releases/download/desktop-latest/Friday_0.1.0_amd64.AppImage) |
...
!!! warning "macOS: \"app is damaged\""
    ...
    ```bash
    xattr -cr /Applications/Friday.app
    ```
    This is normal for open-source apps distributed outside the App Store.

!!! tip "All releases"
    Browse all versions on the [GitHub Releases](https://github.com/purnasai1807-lgtm/Friday_AI/releases) page.
...
The command-line interface is the fastest way to interact with Friday
...
friday ask --agent orchestrator --tools calculator "What is 137 * 42?"
...
For programmatic access, the `Friday` class provides a high-level sync API.
...
```python
from friday import SystemBuilder
...
Friday uses optional extras to keep the base installation lightweight.
...
!!! note "Ollama, vLLM, and llama.cpp are HTTP-based"
    These engines have no additional Python dependencies — Friday communicates over HTTP. You still need the engine software running on your machine.
...
| Extra | Install Command | Description |
|-------|----------------|-------------|
| `server` | `uv sync --extra server` | OpenAI-compatible API server (`friday serve`) |
...
Friday requires at least one inference backend. Choose the one that matches your hardware.
