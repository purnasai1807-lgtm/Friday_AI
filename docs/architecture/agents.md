...
!!! note "Backward compatibility"
    The old `from friday.agents.react import ReActAgent` import path still works via a backward-compat shim. The registry alias `"react"` also maps to `NativeReActAgent`.

```python
from friday.agents.native_react import NativeReActAgent
...
```python
from friday.agents.native_openhands import NativeOpenHandsAgent
...
```python
from friday.agents.rlm import RLMAgent
...
```python
from friday.agents.openhands import OpenHandsAgent
...
1. On first call, copies the bundled `claude_code_runner/` to `~/.friday/claude_code_runner/` and runs `npm install --production` if `node_modules` is absent
...
- `cleanup_orphans()` removes any stale containers labelled `friday-sandbox=true`
...
from friday.agents._stubs import BaseAgent
