...
description: The five-primitive architecture behind Friday — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
Friday is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![Friday Architecture](../assets/Friday_Architecture.png)
...
The sandbox module (`friday.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in Friday use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/friday/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           FridayConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              friday ask -- query the assistant
        serve.py            friday serve -- start API server

    sdk.py              Friday class -- high-level Python SDK
