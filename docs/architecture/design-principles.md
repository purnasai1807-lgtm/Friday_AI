...
Friday follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in Friday is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
Friday is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Friday(engine_key="ollama")  # Local Ollama server
...
Friday **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `friday init` and used as the default engine:

```bash
friday init --force
# Detects hardware, writes ~/.friday/config.toml with:
# [engine]
...
```bash
friday telemetry stats          # Aggregated statistics
friday telemetry export --json  # Export all records
```
...
Friday provides a **clean Python API** through the `Friday` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`friday[server]`, `friday[memory-colbert]`, etc.)
...
The API server (`friday serve`) implements the **OpenAI chat completions API format**, making Friday a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to Friday:
...
Friday requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of Friday.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `friday[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `friday[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `friday[memory-bm25]` | `rank-bm25` |
| API server | `friday[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `friday[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `friday[inference-vllm]` | `vllm` |
| PDF ingestion | `friday[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `friday[channel-whatsapp-baileys]` | Node.js 22+ |
