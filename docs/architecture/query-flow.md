...
This page traces the end-to-end journey of a user query through the Friday system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: friday ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: FridayConfig (hardware, engine defaults)
...
Friday supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
friday ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.friday/config.toml
```
...
- Returns a `FridayConfig` dataclass with all settings
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.friday/telemetry.db`.
...
The `Friday` class in `sdk.py` provides the same query flow through a Python API:

```python
from friday import Friday

j = Friday(model="qwen3:8b", engine_key="ollama")
