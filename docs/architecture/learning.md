...
!!! note "Backward compatibility"
    The canonical locations are now `friday.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `friday.core.types` (for `RoutingContext`). The old `friday.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `friday.learning._stubs`.
...
```bash
friday ask --router learned "What is the capital of France?"
```
...
```python
from friday.learning.router import HeuristicRouter, build_routing_context
...
```python
from friday.learning.router import build_routing_context
...
```python
from friday.learning.trace_policy import TraceDrivenPolicy
from friday.traces.analyzer import TraceAnalyzer
...
```python
from friday.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from friday.learning.sft_policy import SFTPolicy
```
...
```python
from friday.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from friday.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from friday.learning.heuristic_reward import HeuristicRewardFunction
...
store = TraceStore("~/.friday/traces.db")
...
over Friday's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `friday-learning`
