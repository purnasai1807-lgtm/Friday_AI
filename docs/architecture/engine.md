...
The Engine primitive provides the **inference runtime** -- the layer that connects Friday to language model servers. All backends implement a uniform interface, making it straightforward to swap between local and cloud inference without changing application code.
...
Friday automatically detects system hardware to recommend the best engine. Detection runs at config load time via `detect_hardware()`:
...
```python
from friday.engine import discover_engines
...
```python
from friday.engine import discover_engines, discover_models
...
Engine hosts and defaults are configured in `~/.friday/config.toml` using **nested per-engine sub-sections**:
...
```python
from friday.engine._base import messages_to_dicts
...
```python
from friday.engine import EngineConnectionError
