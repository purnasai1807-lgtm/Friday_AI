...
- **Persistence:** Data persists across restarts in `~/.friday/memory.db`
...
```python
from friday.tools.storage.sqlite import SQLiteMemory
from friday.tools.storage.faiss_backend import FAISSMemory
from friday.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from friday.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `friday.tools.storage.*`.
...
```python
from friday.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from friday.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from friday.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.friday/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
