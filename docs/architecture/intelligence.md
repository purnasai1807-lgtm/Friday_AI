...
```python
from friday.core.registry import ModelRegistry
...
```python
from friday.intelligence import merge_discovered_models
...
The following names are still importable from `friday.intelligence` via shim modules, but their canonical locations have moved:
