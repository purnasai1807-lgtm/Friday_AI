...
Recipes are the universal composition format for Friday.  Each recipe
...
_USER_RECIPES_DIR = Path.home() / ".friday" / "recipes"
_USER_OPERATORS_DIR = Path.home() / ".friday" / "operators"
...
class Recipe:
    ...
    def to_builder_kwargs(self) -> Dict[str, Any]:
        """Convert recipe fields to kwargs for SystemBuilder/Friday.

        Returns a dict with only the non-None fields, keyed to match
        the SystemBuilder fluent API or Friday constructor parameters.
        """
        kwargs: Dict[str, Any] = {}
    ...
    def to_eval_suite(
        ...
        """
        from friday.recipes.composer import recipe_to_eval_suite
    ...
    def to_operator_manifest(self) -> Any:
        ...
        from friday.recipes.composer import recipe_to_operator
...
def discover_recipes(
    ...
    *,
    ...
    3. User ``~/.friday/recipes/`` directory
    4. User ``~/.friday/operators/`` directory
