...
from friday.skills.dependency import validate_dependencies
from friday.skills.executor import SkillExecutor, SkillResult
from friday.skills.loader import discover_skills
from friday.skills.tool_adapter import SkillTool
from friday.skills.types import SkillManifest
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.friday/learning/skills/ if config can't be loaded.
            try:
                ...
                pass
            if overlay_dir is None:
                overlay_dir = Path("~/.friday/learning/skills/").expanduser()
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.friday.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from friday.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("friday", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["friday"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.friday.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("friday", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.friday/skills/discovered/``).  Returns a list of dicts with
        ...
        """
        from friday.learning.agents.skill_discovery import SkillDiscovery
        ...
        if output_dir is None:
            output_dir = Path("~/.friday/skills/discovered/").expanduser()
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "friday (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        """
        if roots is None:
            roots = [Path("~/.friday/skills/").expanduser(), Path("./skills")]
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from friday.skills.loader import load_skill_directory
