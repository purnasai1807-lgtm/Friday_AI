...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.friday.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "friday_meta" the value is
# stored under manifest.metadata["friday"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("friday_meta", "platforms"),
    "prerequisites": ("friday_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.friday.original_frontmatter,
        and merges metadata.friday.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize friday namespace
        oj_meta = dict(raw_metadata.get("friday") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.friday.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.friday.* into canonical fields (these override
        # top-level mappings since they are explicit Friday-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.friday.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the friday metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["friday"] = oj_meta
