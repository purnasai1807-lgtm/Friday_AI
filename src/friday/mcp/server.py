"""MCP Server — wraps Friday tools as MCP-discoverable tools."""
...
from friday.mcp.protocol import (
    INTERNAL_ERROR,
    INVALID_PARAMS,
    METHOD_NOT_FOUND,
    MCPRequest,
...
class MCPServer:
    """MCP server that exposes Friday tools via JSON-RPC.
    ...
    def _auto_discover_tools() -> List[BaseTool]:
        ...
        try:
            ...
            pass
        try:
            from friday.tools.code_interpreter import CodeInterpreterTool

            _tool_classes.append(CodeInterpreterTool)
        except ImportError:
            pass
        try:
            from friday.tools.repl import ReplTool
        ...
        # Channel MCP tools
        try:
            from friday.tools.channel_tools import (
                ChannelListTool,
                ChannelSendTool,
                ChannelStatusTool,
            )
