"""``friday config`` — configuration inspection commands."""
...
def _get_config_path(path: str | None) -> Path:
    ...
    from friday.core.config import DEFAULT_CONFIG_PATH
...
def _show_hardware_info(console: Console, show_recommendations: bool = True) -> None:
    ...
    from friday.core.config import detect_hardware, recommend_engine
...
def set_config(key: str, value: str) -> None:
    """Set a configuration value (e.g. friday config set engine.ollama.host URL)."""
    import tomlkit

    from friday.core.config import DEFAULT_CONFIG_DIR, validate_config_key
