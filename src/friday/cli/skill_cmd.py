...
def _get_discovered_dir() -> Path:
    ...
    return Path("~/.friday/skills/discovered/").expanduser()


def _get_overlay_dir() -> Path:
    """Return the directory where optimization overlays are stored."""
    return Path("~/.friday/learning/skills/").expanduser()
...
def _get_skill_paths() -> List[Path]:
    ...
    user_dir = Path("~/.friday/skills/").expanduser()
...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from friday.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from friday.skills.sources.openclaw import OpenClawResolver
    ...
    if source == "github":
        ...
        cache = _Path(
            "~/.friday/skill-cache/github/" + url.rstrip("/").rsplit("/", 1)[-1]
        ).expanduser()
...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``friday skill install hermes:apple-notes``
    """
    console = Console()
    ...
    from friday.skills.importer import SkillImporter
    from friday.skills.parser import SkillParser
    from friday.skills.tool_translator import ToolTranslator
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.friday/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: friday skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.friday/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="friday-discover-dryrun-"))
        try:
