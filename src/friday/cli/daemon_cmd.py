"""``friday start|stop|restart|status`` — daemon management commands."""
...
from friday.core.config import DEFAULT_CONFIG_DIR, load_config
...
@click.group()
def daemon() -> None:
    """Manage the Friday server daemon."""
...
def start(
    ...
    """Start the Friday server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'friday stop' to stop it first, or 'friday restart'.")
        sys.exit(1)
    ...
    # Build command to run friday serve
    cmd = [sys.executable, "-m", "friday.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]Friday server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running Friday server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the Friday server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the Friday server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
