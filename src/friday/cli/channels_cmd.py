"""``friday channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from friday.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "friday channels imessage-start <contact>",
        )
...
def imessage_start(
    ...
    """
    from friday.channels.imessage_daemon import (
        is_running,
        run_daemon,
    )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "friday.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from friday.connectors.retriever import (
            TwoStageRetriever,
        )
        from friday.connectors.store import KnowledgeStore
        from friday.engine.ollama import OllamaEngine
        from friday.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from friday.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from friday.channels.imessage_daemon import stop_daemon
