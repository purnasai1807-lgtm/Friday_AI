"""``friday deep-research-setup`` — auto-detect local sources, ingest, and chat.
...
from friday.connectors.pipeline import IngestionPipeline
from friday.connectors.store import KnowledgeStore
from friday.connectors.sync_engine import SyncEngine
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from friday.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from friday.connectors.imessage import IMessageConnector

        return IMessageConnector(db_path=config.get("db_path", ""))
    elif connector_id == "obsidian":
        from friday.connectors.obsidian import ObsidianConnector

        return ObsidianConnector(vault_path=config.get("vault_path", ""))
    elif connector_id == "gmail_imap":
        from friday.connectors.gmail_imap import GmailIMAPConnector

        return GmailIMAPConnector()
    elif connector_id == "outlook":
        from friday.connectors.outlook import OutlookConnector

        return OutlookConnector()
    elif connector_id == "slack":
        from friday.connectors.slack_connector import SlackConnector

        return SlackConnector()
    elif connector_id == "notion":
        from friday.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from friday.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.friday/sync_state.db`` when empty.
...
def _launch_chat(store: KnowledgeStore, console: Console) -> None:
    ...
    from friday.agents.deep_research import DeepResearchAgent
    from friday.connectors.retriever import TwoStageRetriever
    from friday.engine.ollama import OllamaEngine
    from friday.tools.knowledge_search import KnowledgeSearchTool
    from friday.tools.knowledge_sql import KnowledgeSQLTool
    from friday.tools.scan_chunks import ScanChunksTool
