"""``friday agents`` — persistent agent lifecycle management."""
...
def list_agents() -> None:
    ...
    try:
        ...
        if not agents:
            console.print(
                "[dim]No agents found. Create one with: friday agents create[/dim]"
            )
            return
...
def search(agent_id: str, query: str, limit: int) -> None:
    ...
    try:
        ...
        if not agent:
            console.print(f"[red]Agent not found: {agent_id}[/red]")
            return
        store = TraceStore(config.traces.db_path or "~/.friday/traces.db")
...
def _get_system():
    """Build a FridaySystem for CLI commands that need scheduler/executor."""
...
def _get_scheduler_and_executor(system=None):
    """Get scheduler + executor from a FridaySystem instance."""
    if system is None:
        system = _get_system()
...
def launch():
    ...
    from friday.agents.manager import AgentManager as _AM
...
def learning(agent_id, trigger_run):
    ...
    if trigger_run:
        ...
        from friday.core.events import EventType, get_event_bus
