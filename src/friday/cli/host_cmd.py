"""``friday host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      friday host mlx-community/Qwen2.5-7B-4bit --backend mlx
      friday host Qwen/Qwen3-8B --backend vllm
      friday host qwen3:8b --backend ollama
      friday host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]Friday will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
