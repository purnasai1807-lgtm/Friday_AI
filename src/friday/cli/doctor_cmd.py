"""``friday doctor`` — run diagnostic checks on the Friday installation."""
...
from friday.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `friday init` to generate a config file.",
    )
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "friday[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "friday[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "friday[energy-amd]", "AMD energy monitoring"),
        ("colbert", "friday[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "friday[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your Friday installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]Friday Doctor[/bold]")
    console.print()
