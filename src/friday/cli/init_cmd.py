"""``friday init`` — detect hardware, generate config, write to disk."""
...
from friday.cli.model import find_model_spec, hf_download, ollama_pull
from friday.cli.scan_cmd import PrivacyScanner
...
# Engines supported by ``friday init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     friday ask "Hello"\n'
            "\n"
            "  Run `friday doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]friday scan[/cyan] for a full environment audit.")
...
def init(
    ...
    """Detect hardware and generate ~/.friday/config.toml."""
    console = Console()
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "friday" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]friday doctor[/bold] to verify."
        )
        return
...
"""
        ...
        console.print(
            ...
            "Run [bold]friday connect gdrive[/bold] to connect "
            "Google services, then [bold]friday digest --fresh[/bold]."
        )
    ...
    if not model:
        ...
        from friday.core.config import _available_memory_gb
        ...
        if not no_download and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]friday model pull {model}[/bold]"
                )
