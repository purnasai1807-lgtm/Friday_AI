"""``friday serve`` — OpenAI-compatible API server."""
...
from friday.engine import (
    discover_engines,
    discover_models,
    get_engine,
)
from friday.intelligence import (
...
def serve(
    ...
    if _has_cloud and engine_name != "cloud":
        try:
            from friday.engine.cloud import CloudEngine
            from friday.engine.multi import MultiEngine
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    import friday.tools  # noqa: F401  # trigger registration
    ...
    if not api_key:
        try:
            ...
            _cfg_path = str(
                __import__("pathlib").Path.home() / ".friday" / "config.toml"
            )
    ...
    from friday.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from friday.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    if channel_bridge is not None:
        try:
            from friday.server.channel_bridge import (
                ChannelBridge,
            )
            from friday.server.session_store import (
                SessionStore,
            )
    ...
    console.print(
        f"[green]Starting Friday API server[/green]\n"
