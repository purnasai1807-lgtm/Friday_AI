...
A thin wrapper over a local git repository at ``<friday_home>/.git``.
...
from friday.learning.distillation.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.friday/`` (or ``$OPENJARVIS_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the Friday source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"Friday source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "distillation@friday.local")
            self._git("config", "user.name", "Friday Distillation")
        ...
        # Allow empty so init succeeds even on a brand-new friday home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
