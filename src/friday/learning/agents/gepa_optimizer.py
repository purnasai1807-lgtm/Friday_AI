...
Uses GEPA's adapter pattern to bridge Friday traces into GEPA's
...
from friday.core.config import GEPAOptimizerConfig
from friday.core.registry import LearningRegistry
from friday.learning._stubs import AgentLearningPolicy
...
class FridayGEPAAdapter:
    """Implements GEPA's adapter protocol for Friday agents.
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        if not HAS_GEPA:
            return {
                "status": "error",
                "reason": (
                    "gepa not installed (pip install 'friday[learning-gepa]')"
                ),
            }

        agent_name = self.config.agent_filter or "default"
        adapter = FridayGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "FridayGEPAAdapter"]
