"""Single point of contact between Python and the Rust ``friday_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``friday_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    ...
    """
    import friday_rust  # type: ignore[import-untyped]
...
def scan_result_from_json(json_str: str) -> object:
    ...
    from friday.security.types import (
        ScanFinding,
        ScanResult,
        ThreatLevel,
    )
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from friday.security.injection_scanner import (
        InjectionScanResult,
    )
    from friday.security.types import ScanFinding, ThreatLevel
...
def retrieval_results_from_json(json_str: str) -> list:
    ...
    from friday.tools.storage._stubs import RetrievalResult
