"""FastAPI application factory for the Friday API server."""
...
from friday.server.api_routes import include_all_routes
from friday.server.comparison import comparison_router
from friday.server.connectors_router import create_connectors_router
from friday.server.dashboard import dashboard_router
from friday.server.digest_routes import create_digest_router
from friday.server.routes import router
from friday.server.upload_router import router as upload_router
        ...
        for agent in mgr.list_agents():
            ...
            for b in bindings:
                ...
                if bridge and hasattr(bridge, "_channels"):
                    ...
                    from friday.server.channel_bridge import ChannelBridge
                    from friday.server.session_store import SessionStore

                    session_store = SessionStore()
                    engine = getattr(app.state, "engine", None)
                    dr_agent = None
                    if engine:
                        from friday.server.agent_manager_routes import (
                            _build_deep_research_tools,
                        )

                        tools = _build_deep_research_tools(engine=engine, model="")
                        if tools:
                            from friday.agents.deep_research import (
                                DeepResearchAgent,
                            )
...
def create_app(
    ...
    *,
    ...
    config:
        Optional FridayConfig for other settings.
    include_cortex:
        When ``True`` (default), include the autonomous multi-agent Cortex
        subsystem routes under ``/v1/cortex``.
    """
    app = FastAPI(
        title="Friday API",
        description="OpenAI-compatible API server for Friday",
        version="0.1.0",
    )
    ...
    # Include the Cortex multi-agent subsystem routes (additive, opt-in).
    if include_cortex:
        try:
            from friday.server.cortex_routes import create_cortex_router

            app.include_router(create_cortex_router())
        except Exception as exc:
            logger.debug("Cortex routes init skipped: %s", exc)
    ...
    try:
        from friday.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from friday.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from friday.server.webhook_routes import (
