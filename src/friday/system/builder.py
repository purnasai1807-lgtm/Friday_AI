"""Config-driven fluent builder that wires up a FridaySystem."""
...
from friday.core.config import FridayConfig, load_config
from friday.core.events import EventBus, get_event_bus
...
class SystemBuilder:
    """Config-driven fluent builder for FridaySystem."""

    def __init__(
        self,
        config: Optional[FridayConfig] = None,
        *,
    ...
    def build(self) -> FridaySystem:
        """Construct a fully wired FridaySystem."""
        config = self._config
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            try:
                from friday.telemetry.energy_monitor import (
            ...
            if energy_monitor is None:
                try:
                    from friday.telemetry.gpu_monitor import GpuMonitor
        ...
        from friday.security import setup_security

        sec = setup_security(config, engine, bus)
        engine = sec.engine

        if telemetry_enabled:
            from friday.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
        ...
        if config.agent_manager.enabled:
            try:
                ...
                from friday.agents.manager import AgentManager

                am_db = config.agent_manager.db_path or str(
                    Path("~/.friday/agents.db").expanduser()
                )
        ...
        if agent_manager is not None:
            try:
                from friday.agents.executor import AgentExecutor
                from friday.agents.scheduler import AgentScheduler
        ...
        if speech_enabled:
            try:
                from friday.speech._discovery import get_speech_backend
    ...
    def _resolve_engine(self, config: FridayConfig):
        from friday.engine._discovery import get_engine
    ...
    def _resolve_model(self, config: FridayConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _setup_telemetry(self, config, bus):
        try:
            from friday.telemetry.store import TelemetryStore
    ...
    def _resolve_memory(self, config):
        try:
            import friday.tools.storage  # noqa: F401 -- trigger registration
            from friday.core.registry import MemoryRegistry
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import friday.channels  # noqa: F401 -- trigger registration
            from friday.core.registry import ChannelRegistry
            from friday.system._channel_kwargs import build_channel_kwargs
    ...
    def _setup_sandbox(self, config):
        ...
        if not sandbox_enabled:
            return None
        try:
            from friday.sandbox.runner import ContainerRunner
    ...
    def _setup_scheduler(self, config, bus):
        ...
        try:
            ...
            if not config.scheduler.db_path:
                from friday.core.config import DEFAULT_CONFIG_DIR
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: FridayConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from friday.core.config import DEFAULT_CONFIG_DIR
            from friday.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from friday.learning.training.lora import LoRATrainingConfig
            from friday.traces.store import TraceStore
    ...
    def _discover_external_mcp(self, server_cfg) -> List[BaseTool]:
        ...
        from friday.mcp.transport import StdioTransport, StreamableHTTPTransport
        from friday.tools.mcp_adapter import MCPToolProvider
