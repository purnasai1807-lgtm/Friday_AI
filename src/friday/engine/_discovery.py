...
from friday.engine._base import InferenceEngine
...
def _make_engine(key: str, config: FridayConfig) -> InferenceEngine:
...
def discover_engines(config: FridayConfig) -> List[Tuple[str, InferenceEngine]]:
...
def get_engine(
    config: FridayConfig, engine_key: str | None = None
