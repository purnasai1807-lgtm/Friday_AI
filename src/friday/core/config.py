...
User configuration lives at ``~/.friday/config.toml``.  ``load_config()``
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from friday.intelligence.model_catalog import BUILTIN_MODELS
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.friday/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "hardware through Friday. You are not a cloud service. Respond "
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.friday/whatsapp_auth
...
class SandboxConfig:
    ...
    enabled: bool = False
    image: str = "friday-sandbox:latest"
    timeout: int = 300
    workspace: str = ""
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.friday/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.friday/operators"
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.friday/SOUL.md"
    memory_path: str = "~/.friday/MEMORY.md"
    user_path: str = "~/.friday/USER.md"
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.friday/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/friday/skill-index.git"
    index_dir: str = "~/.friday/skill-index/"
    max_depth: int = 5
...
@dataclass
class FridayConfig:
    """Top-level configuration for Friday."""
...
# Sections that users may set via ``friday config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(FridayConfig.__dataclass_fields__.keys()) - {"hardware"}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``FridayConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import friday.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "FridayConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> FridayConfig:
    ...
    path:
        ...
        otherwise ``~/.friday/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = FridayConfig(hardware=hw)
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``friday init``)
# ---------------------------------------------------------------------------
...
# Full reference config: friday init --full
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.friday/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.friday/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.friday/whatsapp_auth
