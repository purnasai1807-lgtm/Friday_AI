"""Friday Agent backend — agent-level inference with tool calling."""
...
class FridayAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + FridaySystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "friday-agent"
    ...
    def __init__(
        ...
        # Override the agent's per-run turn budget. FridayConfig.agent.max_turns
