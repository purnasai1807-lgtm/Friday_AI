...
from friday.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a FridaySystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after FridaySystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from friday.agents import AgentRegistry
        ...
        # Resolve engine + model from FridaySystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in FridaySystem")
        ...
        if router_policy_key and self._system:
            try:
                from friday.core.registry import RouterPolicyRegistry
                from friday.learning.routing.types import (
    ...
    def _save_trace(
        ...
        from friday.core.types import StepType, Trace, TraceStep
