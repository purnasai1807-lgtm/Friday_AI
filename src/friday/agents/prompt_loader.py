...
def _friday_home() -> Path:
    """Resolve $OPENJARVIS_HOME, defaulting to ~/.friday."""
    return Path(os.environ.get("OPENJARVIS_HOME", "~/.friday")).expanduser()
...
def load_system_prompt_override(agent_name: str) -> str | None:
    ...
    ``OPENJARVIS_HOME`` defaults to ``~/.friday`` when unset.
    """
    home = _friday_home()
