...
//!
//! Rust translation of `src/friday/core/config.py`.
...
/// Default config directory: `~/.friday/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".friday")
}

/// Default config file path: `~/.friday/config.toml`
...
fn default_sandbox_image() -> String { "friday-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.friday/operators".into() }
...
pub fn load_config(path: Option<&Path>) -> Result<FridayConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: FridayConfig = toml::from_str(&content)?;
...
mode = "block"
"#;
        let cfg: FridayConfig = toml::from_str(toml_str).unwrap();
