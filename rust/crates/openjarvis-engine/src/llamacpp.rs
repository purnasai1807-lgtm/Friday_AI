...
use friday_core::error::{EngineError, FridayError};
use friday_core::{GenerateResult, Message, Usage};
...
impl InferenceEngine for LlamaCppEngine {
    ...
    fn generate(
        ...
        max_tokens: i64,
        _extra: Option<&Value>,
    ) -> Result<GenerateResult, FridayError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|e| {
                FridayError::Engine(EngineError::Connection(format!(
        ...
        if !resp.status().is_success() {
            ...
            return Err(FridayError::Engine(EngineError::Http(format!(
                "llama.cpp returned {}: {}",
                status, body
            ))));
        }
        ...
        let data: Value = resp.json().map_err(|e| {
            FridayError::Engine(EngineError::Deserialization(e.to_string()))
        })?;
    ...
    async fn stream(
        ...
        max_tokens: i64,
        _extra: Option<&Value>,
    ) -> Result<TokenStream, FridayError> {
        let prompt = Self::messages_to_prompt(messages);
        let payload = serde_json::json!({
            "prompt": prompt,
            "n_predict": max_tokens,
        ...
        let async_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                FridayError::Engine(EngineError::Connection(e.to_string()))
            })?;
        ...
        let token_stream = byte_stream.filter_map(|chunk_result| async {
            match chunk_result {
                ...
                Err(e) => Some(Err(FridayError::Engine(EngineError::Streaming(
                    e.to_string(),
                )))),
            }
        });
    ...
    fn list_models(&self) -> Result<Vec<String>, FridayError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|_| {
                FridayError::Engine(EngineError::Connection(
