...
use friday_core::config::FridayConfig;
...
pub fn discover_engines(config: &FridayConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, FridayError> {
    ...
    match key.as_str() {
        ...
        other => Err(FridayError::Engine(
            friday_core::error::EngineError::ModelNotFound(format!(
                "Unknown engine: {}",
                other
            )),
        )),
