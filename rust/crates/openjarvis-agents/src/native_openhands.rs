...
use friday_core::{AgentContext, AgentResult, FridayError, ToolResult};
use friday_tools::executor::ToolExecutor;
use regex::Regex;
    ...
    async fn run(
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        friday_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        friday_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    FridayError::Agent(friday_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<friday_engine::Engine>>;
