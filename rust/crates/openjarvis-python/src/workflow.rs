...
pub struct PyWorkflowGraph {
    inner: friday_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: friday_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => friday_workflow::NodeType::Tool,
            "condition" => friday_workflow::NodeType::Condition,
            "parallel" => friday_workflow::NodeType::Parallel,
            "loop" => friday_workflow::NodeType::Loop,
            "transform" => friday_workflow::NodeType::Transform,
            _ => friday_workflow::NodeType::Agent,
        };
        let node = friday_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = friday_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<friday_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(friday_workflow::WorkflowBuilder::new(name)),
        }
    }
