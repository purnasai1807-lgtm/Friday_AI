...
pub struct PySecretScanner {
    inner: friday_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: friday_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: friday_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: friday_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: friday_security::GuardrailsEngine<friday_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => friday_security::RedactionMode::Redact,
            "block" => friday_security::RedactionMode::Block,
            _ => friday_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: friday_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<friday_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = friday_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: friday_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: friday_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: friday_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: friday_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: friday_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: friday_security::RateLimiter::new(
                friday_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
pub struct PyTaintSet {
    inner: friday_security::TaintSet,
}
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: friday_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => friday_security::TaintLabel::Pii,
            "secret" => friday_security::TaintLabel::Secret,
            "user_private" => friday_security::TaintLabel::UserPrivate,
            "external" => friday_security::TaintLabel::External,
            _ => friday_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &friday_security::TaintSet::from_labels(&[taint_label]),
        );
    }
