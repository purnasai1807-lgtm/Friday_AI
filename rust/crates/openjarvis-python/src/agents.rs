...
use crate::RUNTIME;
use friday_agents::OjAgent;
use friday_engine::rig_adapter::RigModelAdapter;
use friday_engine::Engine;
use pyo3::prelude::*;
use std::sync::Arc;
...
enum AgentEnum {
    Simple(friday_agents::SimpleAgent<DefaultAdapter>),
    Orchestrator(friday_agents::OrchestratorAgent<DefaultAdapter>),
    NativeReAct(friday_agents::NativeReActAgent<DefaultAdapter>),
}
...
impl AgentEnum {
    ...
    async fn run(
        &self,
        input: &str,
        context: Option<&friday_core::AgentContext>,
    ) -> Result<friday_core::AgentResult, friday_core::FridayError> {
        match self {
...
fn make_adapter(engine_key: &str, model: &str) -> PyResult<DefaultAdapter> {
    let config = friday_core::FridayConfig::default();
    let engine = friday_engine::get_engine_static(&config, Some(engine_key))
...
impl PySimpleAgent {
    ...
    fn new(
        ...
        let agent = friday_agents::SimpleAgent::new(adapter, system_prompt, temperature);
...
impl PyOrchestratorAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(friday_tools::ToolExecutor::new(None, None));
        let agent = friday_agents::OrchestratorAgent::new(
...
impl PyNativeReActAgent {
    ...
    fn new(
        ...
        let adapter = make_adapter(engine_key, model)?;
        let executor = Arc::new(friday_tools::ToolExecutor::new(None, None));
        let agent = friday_agents::NativeReActAgent::new(
...
impl PyNativeOpenHandsAgent {
    ...
    fn new(
        ...
        let engine = friday_engine::get_engine_static(&config, Some(engine_key))
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        let adapter = friday_engine::rig_adapter::RigModelAdapter::new(
            Arc::new(engine),
            model.to_string(),
        );
        let executor = Arc::new(friday_tools::ToolExecutor::new(None, None));
        let agent = friday_agents::NativeOpenHandsAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
impl PyMonitorOperativeAgent {
    ...
    fn new(
        ...
        let mem_ext = match memory_extraction {
            "scratchpad" => friday_agents::MemoryExtraction::Scratchpad,
            "structured_json" => friday_agents::MemoryExtraction::StructuredJson,
            "none" => friday_agents::MemoryExtraction::None,
            _ => friday_agents::MemoryExtraction::CausalityGraph,
        };
        let obs_comp = match observation_compression {
            "truncate" => friday_agents::ObservationCompression::Truncate,
            "none" => friday_agents::ObservationCompression::None,
            _ => friday_agents::ObservationCompression::Summarize,
        };
        let ret_strat = match retrieval_strategy {
            "keyword" => friday_agents::RetrievalStrategy::Keyword,
            "semantic" => friday_agents::RetrievalStrategy::Semantic,
            "none" => friday_agents::RetrievalStrategy::None,
            _ => friday_agents::RetrievalStrategy::HybridWithSelfEval,
        };
        let task_dec = match task_decomposition {
            "monolithic" => friday_agents::TaskDecomposition::Monolithic,
            "hierarchical" => friday_agents::TaskDecomposition::Hierarchical,
            _ => friday_agents::TaskDecomposition::Phased,
        };

        let monitor_config = friday_agents::MonitorConfig {
        ...
        let agent = friday_agents::MonitorOperativeAgent::new(
            adapter,
            executor,
            max_turns,
            temperature,
...
impl PyLoopGuard {
    ...
    fn new(max_identical: usize, max_ping_pong: usize, poll_budget: usize) -> Self {
        Self {
            inner: friday_agents::LoopGuard::new(max_identical, max_ping_pong, poll_budget),
        }
    }
