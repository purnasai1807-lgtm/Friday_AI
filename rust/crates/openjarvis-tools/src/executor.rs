...
use friday_core::error::{FridayError, ToolError};
use friday_core::{EventBus, EventType, ToolResult};
use friday_security::capabilities::CapabilityPolicy;
use friday_security::taint::{TaintSet, check_taint};
...
impl ToolExecutor {
    ...
    pub fn execute(
    ...
    ) -> Result<ToolResult, FridayError> {
        let tool = self.tools.get(tool_name).ok_or_else(|| {
            FridayError::Tool(ToolError::NotFound(tool_name.to_string()))
        })?;
        ...
        if let (Some(policy), Some(aid)) = (&self.capability_policy, agent_id) {
            ...
            for cap in &spec.required_capabilities {
                if !policy.check(aid, cap, "") {
                    return Err(FridayError::Tool(ToolError::CapabilityDenied(
                        aid.to_string(),
        ...
        if let Some(taint_set) = taint {
            if let Some(violation) = check_taint(tool_name, taint_set) {
                return Err(FridayError::Tool(ToolError::TaintViolation(
                    tool_name.to_string(),
                    violation,
                )));
            }
        ...
        if elapsed > timeout {
            ...
            return Err(FridayError::Tool(ToolError::Timeout(
                timeout.as_secs_f64(),
                tool_name.to_string(),
            )));
        }
...
mod tests {
    ...
    fn test_executor_tool_not_found() {
        ...
        let err = exec
            ...
            .unwrap_err();
        assert!(matches!(err, FridayError::Tool(ToolError::NotFound(_))));
    }
}
